...
- Features added:
  - [sweeping] A new generic measurement that sweep a settings and collects data by either invoking read_from_hardware from any applicable setting, running any measurement (here, data in the final h5 are collected from the measurements .data dict if defined), or subclasses of Collector that allow more specialized data collection. Can be subclassed or used as a standalone.
  - [favorites_widget] can now dynamically add the default widget of a settings above the tree panel.
  - [hardware] added properties and attributes: has_been_connected_once, is_connected, toggle_to_connected_count. Added read_from_hardware action to the tree.
  - [measurement] added show buttons to the tree.
  - [app settings] added wheel manipulation. When unchecked, the mouse wheel does no longer manipulate widget connected to LoggedQuantity using connect_to_widget. Resolves Issue #69
  - [intervaled ranges] allows to define ranges with varying densisities of data points.
  - [analyze with ipynb] now appends a cell in the notebook that loads the latest measurement.
