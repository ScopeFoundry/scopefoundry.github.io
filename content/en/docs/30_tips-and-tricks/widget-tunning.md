...
`connect_to_widget` configures the widget's behavior based on the attributes of `my_setting`, as discussed here. This also affects the the convince methods `New_UI` and `new_default_widget`, see [default widgets](#default-widgets).
...
If the setting takes a discrete set of values, it is often wise to specify choices. If choices are defined, `QtWidgets.QComboBox` becomes the default widget. 
...
- Displayed and underlying values are different:
  ...
  To update it, you would have to use the underlying value:
...
- To connect a bool `lq` to a QPushButton `button`, use:
  ```python
...
```python
self.settings.New("pi", int, initial=3, description="the ratio of circumference to diameter is 3.0 by law")
```
...
The `default_widget` widget is determined by the `dtype` of the setting. Refer to the [compatibility matrix](#compatibility-matrix) for the default widget associated with each `dtype`.

{{% pageinfo color="info" %}}
New in ScopeFoundry 2.0
{{% /pageinfo %}}

You can override the default widget at the time of definition using the `default_widget_factory` argument. For example, the [compatibility matrix](#compatibility-matrix) tells us that `int` can be used  with `QtWidgets.QSlider` as an alternative default widget:
